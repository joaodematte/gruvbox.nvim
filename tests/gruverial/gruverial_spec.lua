require("plenary.reload").reload_module("gruverial", true)
local gruverial = require("gruverial")
local default = gruverial.config
...
describe("tests", function()
  it("works with default values", function()
    gruverial.setup()
    assert.are.same(gruverial.config, default)
  end)
  ...
  it("works with config overrides", function()
    ...
    gruverial.setup({ undercurl = false, underline = false })
    assert.are.same(gruverial.config, expected)
  end)
  ...
  it("does not set terminal colors when terminal_colors is false", function()
    clear_term_colors()
    gruverial.setup({ terminal_colors = false })
    gruverial.load()
  ...
  it("sets terminal colors when terminal_colors is true", function()
    clear_term_colors()
    gruverial.setup({ terminal_colors = true })
    gruverial.load()

    -- dark bg
    local colors = require("gruverial").palette
